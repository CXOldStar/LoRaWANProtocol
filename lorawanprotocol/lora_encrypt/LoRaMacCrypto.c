...
void LoRaPingRandencrypt(uint8_t *key, uint32_t beacon_time, uint32_t dev_addr, uint8_t *encBuffer)
{
    uint16_t i;
    uint8_t bufferIndex = 0;
    ...
    aBlock[15] = ( beacon_time >> 24 ) & 0xFF;
    aBlock[14] = ( beacon_time >> 16 ) & 0xFF;
    aBlock[13] = ( beacon_time >> 8 ) & 0xFF;
    aBlock[12] = ( beacon_time ) & 0xFF;
    aBlock[11] = ( dev_addr ) & 0xFF;
    aBlock[10] = ( dev_addr >> 8 ) & 0xFF;
    aBlock[9] = ( dev_addr >> 16 ) & 0xFF;
    aBlock[8] = ( dev_addr >> 24 ) & 0xFF;

    aes_encrypt( aBlock, sBlock, &AesContext );
    for( i = 0; i < 16; i++ )
    {
        encBuffer[bufferIndex + i] = sBlock[bufferIndex + i];
